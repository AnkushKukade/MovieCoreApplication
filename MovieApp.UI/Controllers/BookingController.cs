...
namespace MovieApp.UI.Controllers
{
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> BookingReg(int movieId)
        {
            ...
            {
                string endpoint = _configuration["WebApiBaseUrl"] + "Movie/GetSpecificMovie?id=" + movieId;
            ...
            using (HttpClient client = new HttpClient())
            {
                string endPoint = _configuration["WebApiBaseUrl"] + "MovieShowTime/GetShowTimesAndDateForSpecificTheatreAndMovie?mId=" + movieId;
                using (var response = await client.GetAsync(endPoint))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        using (HttpClient client1 = new HttpClient())
                        {
                            ...
                            using (var response1 = await client1.GetAsync(endpoint1))
                            {
                                if (response1.StatusCode == System.Net.HttpStatusCode.OK)
                                {
                                    ...
                                    {
                                        SelectListItem selectListItem2 = new SelectListItem();
                                        var theatre = theatreModelsList.Where(obj => obj.TheatreId == item.TheatreId).ToList()[0];
                                        selectListItem2.Text = theatre.TheatreName;
        ...
        public async Task<IActionResult> GetBookings()
        {
            using (HttpClient client = new HttpClient())
            {
                string endpoint = _configuration["WebApiBaseUrl"] + "Booking/GetAllBookings";
                using (var response = await client.GetAsync(endpoint))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
